    ...
    <div class="table-responsive table--no-card m-b-40" style="margin-top:4%;">
        <form action="{{ route('board.update', ['id'=>$wiz_boards->id]) }}" method="post">        
            ...
            <p style="font-family: 'Do Hyeon', sans-serif; font-size:30px; color:gray;">제목</p>
                <input type="text" class="form-control" name="title" value="{{$wiz_boards->title}}" required> <br>
            @error('title')
            ...
            <p style="font-family: 'Do Hyeon', sans-serif; font-size:30px; color:gray;">내용</p>
                <textarea name="content" class="form-control" value="{{old('content')}}" rows="10" required>{{$wiz_boards->content}}</textarea> <br>
            @error('content')
            <div class="alert alert-danger">
                {{$message}}
            </div>
